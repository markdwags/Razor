...
__declspec(dllexport) DWORD __stdcall Load( const char *exe, const char *dll, const char *func, const void *dllData, int dataLen, DWORD *pid )
{
	...
	hFile = CreateFile( exe, FILE_GENERIC_READ, FILE_SHARE_WRITE|FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL );
	if ( !hFile )
		return NO_OPEN_EXE;
    ...
    ioh = (IMAGE_OPTIONAL_HEADER *)((DWORD)((IMAGE_FILE_HEADER *)(((DWORD)idh) + idh->e_lfanew + 4)) + sizeof(IMAGE_FILE_HEADER));
    
	Entry = ioh->ImageBase + ioh->AddressOfEntryPoint; // ok we got the exe's entry point (first thing called, before main or WinMain)
	...
	DWORD allocSize = dllNameLen + funcNameLen + dataLen + LoadAsmSize;
	...
	if ( !VirtualProtectEx( ProcInfo.hProcess, (void*)Entry, 8, PAGE_EXECUTE_READWRITE, &Num ) )
	{
		TerminateProcess( ProcInfo.hProcess, 0 );
		return NO_VPROTECT;
	}
	...
	if ( !ReadProcessMemory( ProcInfo.hProcess, (void*)Entry, buff, 8, &Num ) || Num != 8 )
	{
		TerminateProcess( ProcInfo.hProcess, 0 );
		return NO_READ;
	}
	...
	*((DWORD*)&EntryAsm[1]) = ProcMem - (Entry + 5);

	// write the new code at the entry point
	if ( !WriteProcessMemory( ProcInfo.hProcess, (void*)Entry, EntryAsm, 8, &Num ) || Num != 8 )
	{
		TerminateProcess( ProcInfo.hProcess, 0 );
		return NO_WRITE;
	}
	...
	*((DWORD*)(LoadAsm+ 1)) = Entry; // data dest
	...
	*((DWORD*)(LoadAsm+52)) = Entry - (ProcMem + 56); // entry points

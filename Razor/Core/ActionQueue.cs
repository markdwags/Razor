		...
		private static void DropCurrent()
		{
			...
			if ( m_Holding.IsItem )
			{
				if ( World.Player.Backpack != null )
					Client.Instance.SendToServer( new DropRequest( m_Holding, Point3D.MinusOne, World.Player.Backpack.Serial ) ); 
				else
			...
			if ( m_Pending != Serial.Zero )
			{
				if ( m_Lifted + TimeSpan.FromMinutes( 2 ) < DateTime.UtcNow )
				{
					...
					if ( World.Player != null )
					{
						...
						if ( World.Player.Backpack != null )
							Client.Instance.SendToServer( new DropRequest( m_Pending, Point3D.MinusOne, World.Player.Backpack.Serial ) ); 
						else
							Client.Instance.SendToServer( new DropRequest( m_Pending, World.Player.Position, Serial.Zero ) ); 
					}

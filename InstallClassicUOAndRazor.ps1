...
Invoke-WebRequest -Uri "https://github.com/andreakarasho/ClassicUO/releases/download/ClassicUO-dev-preview/ClassicUO-dev-preview-release.zip" -OutFile ".\ClassicUO-dev-preview-release.zip" -Verbose
Write-Host "..done"

# Now download the latest Razor build from GitHub
Write-Host "Downloading the latest dev build of Razor.."
Invoke-WebRequest -Uri "https://github.com/markdwags/Razor/releases/download/Razor-dev-preview/Razor-dev-preview.zip" -OutFile ".\Razor-dev-preview.zip" -Verbose
Write-Host "..done"

# Extract both ClassicUO and Razor into the folder this script is running in
Write-Host "Extracting ClassicUO and Razor"
Expand-Archive -Path ".\ClassicUO-dev-preview-release.zip" -DestinationPath ".\" -Force
Expand-Archive -Path ".\Razor-dev-preview.zip" -DestinationPath ".\Razor\" -Force
Write-Host "..done"
...
if ($(Test-Path ".\settings.json" -PathType Leaf) -eq $False) {
  ...
  Start-Process -FilePath ".\ClassicUO.exe" -Wait
  ...
  $cuoSettings = Get-Content ".\settings.json" -Raw | ConvertFrom-Json
  ...
  $cuoSettings | ConvertTo-Json -Depth 32 | Set-Content ".\settings.json"
...
if ($(Test-Path "$Home\Desktop\ClassicUO.lnk" -PathType Leaf) -eq $False) {
  ...
  if ($createShortcut -eq "Y" -Or $createShortcut -eq "y") {
    ...
    $shortcut.TargetPath = "$PSScriptRoot\ClassicUO.exe"
    $shortcut.Save()
  }  
}
...
if ($launch -eq "Y" -Or $launch -eq "y") {
  ...
  Start-Process -FilePath ".\ClassicUO.exe"
}
